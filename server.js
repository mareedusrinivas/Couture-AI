...
const isMockMode = API_KEY === "YOUR_HUGGING_FACE_API_KEY";

if (isMockMode) {
    console.warn("⚠️ WARNING: Running in MOCK mode. The server will serve placeholder images.");
} else {
    console.log("🔑 Hugging Face API Key detected. Server running in Production/HF mode.");
}
...
app.post("/generate", async (req, res) => {
    ...
    // Automatically enhance prompt to ensure a human model is wearing the clothing in a professional catalog shoot
    ...
    if (isMockMode) {
        console.log("🎨 [Mock Mode] Fetching a mock placeholder image...");
        try {
            ...
            console.log("✅ Mock image retrieved successfully!");
            res.setHeader("Content-Type", "image/jpeg");
            return res.send(Buffer.from(imageBuffer));
        } catch (error) {
            console.warn("⚠️ Mock fetch failed, falling back to offline SVG generation. Error:", error.message);
    ...
    try {
        ...
        console.log(`🎲 Using generation seed: ${seed}`);
        ...
        console.warn("⚠️ Hugging Face API failed, falling back to offline SVG generation. Error:", error.message);
        const svgContent = generateFallbackSVG(inputs);
        res.setHeader("Content-Type", "image/svg+xml");
        return res.send(Buffer.from(svgContent));
    }
